...
pretty_configs = ["TF", "TF+M", "TFA", "TFA+M", "TI", "TI+M"] 
...
def get_speedup(w, c):
	baseline = 1
	if c == "TFM":
		baseline = int(data[w]["TF"])
	elif c == "TFAM":
		baseline = int(data[w]["TFA"])
	elif c == "TIM":
		baseline = int(data[w]["TI"])
	else:
		pass
...
for w in workloads :
    ...
    base_runtime = int(data[w][baseline])
    if w == "Memcached":
        label_text = w + "\n(OOM)"
    elif base_runtime == 1:
        label_text = w + "\n(XXX)"
    ...
    for c in configs:
        ...
        if "M" in c and w != "Memcached" and val != 0:
