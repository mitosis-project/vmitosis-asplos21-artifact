...
pretty_configs = ["TLL", "TRRI", "TRRI+e", "TRRI+g", "TRRI+M"]
...
for w in workloads :
    ...
    if w == "BTree" or w == "Memcached":
        base_runtime = "(OOM)"
    elif data[w][baseline] == 1:
        base_runtime = "(XXX)"
    else:
        base_runtime = "(" + str(int(data[w][baseline])) + "s)"
    max_runtime = data[w]["TRRI"]
    ...
    for c in configs :
        ...
        if c == "TRRIM" and w != "BTree" and w != "Memcached" and val != 0:
